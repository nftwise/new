...
import { GoogleAnalyticsConnector } from '@/lib/api/google-analytics';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const cronSecret = process.env.CRON_SECRET;
    // Skip auth if valid secret is provided
...
async function runRollup(date?: string) {
  ...
  try {
    let targetDate = date;
    if (!targetDate) {
      // Use US timezone for yesterday's date (business data is US-based)
      targetDate = getUSYesterday();
    ...
    console.log(`🌙 [Rollup] Starting 59-metric rollup for ${targetDate} (US date)`);
