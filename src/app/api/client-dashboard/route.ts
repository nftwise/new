...
import { requireClientAccess, handleAuthError } from '@/lib/auth/api-auth';
import { validateApiRequest } from '@/lib/middleware/api-security';
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const { error: validationError, params } = validateApiRequest(request, {
      rateLimit: 'api',
    ...
    // Verify user has access to this client (admin can access all, clients only their own)
    ...
    // Step 7: Get assigned team members
    ...
    const teamMembers = (teamAssignments || []).map((assignment: any) => ({
      id: assignment.team_members.id,
      name: assignment.team_members.name,
      role: assignment.team_members.role,
      specialties: assignment.team_members.specialties,
      avatarUrl: assignment.team_members.avatar_url,
