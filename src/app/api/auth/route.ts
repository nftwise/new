import NextAuth from 'next-auth'
  ...
  email: credentials?.email });
          ...
          try {
            // Read clients from JSON file (development 
  authentication)
  ...
  'utf8');
            ...
            console.log('📊 Total clients loaded:',
  ...
  credentials.email);
            ...
            console.log('👤 User found:', { id: user.id, email:
   user.email, company: user.companyName });
  ...
  user.googleAnalyticsPropertyId,
    ...
    callbacks: {
      async jwt({ token, user }) {
        if (user) {
          token.googleAnalyticsPropertyId =
  user.googleAnalyticsPropertyId;
      ...
      async session({ session, token }) {
        if (session.user) {
          session.user.googleAnalyticsPropertyId =
  token.googleAnalyticsPropertyId as string;
          session.user.googleAdsCustomerId =
  token.googleAdsCustomerId as string;
          session.user.callrailAccountId =
  token.callrailAccountId as string;
        }
        return session;
      }
    },
