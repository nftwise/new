...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    console.log('[GSC OAuth] Tokens received for client:', clientId);
    console.log('[GSC OAuth] Has refresh token:', !!tokens.refresh_token);
    ...
    try {
      ...
      console.log('[GSC OAuth] Connected Google account:', userEmail);
    } catch (emailError) {
      console.error('[GSC OAuth] Could not fetch user email:', emailError);
    }
    ...
    const tokenFile = path.join(tokenDir, `${clientId}-gsc.json`);
    fs.writeFileSync(tokenFile, JSON.stringify(tokens, null, 2));

    console.log('[GSC OAuth] Tokens saved to:', tokenFile);
    ...
    if (userEmail) {
      try {
        ...
        const { error: updateError } = await supabase
          ...
          .update({ gsc_connected_email: userEmail })
          .eq('client_id', clientId);

        if (updateError) {
          console.error('[GSC OAuth] Could not save connected email:', updateError);
        } else {
          console.log('[GSC OAuth] Saved connected email to database');
        }
      } catch (dbError) {
        console.error('[GSC OAuth] Database error:', dbError);
      }
    }
