...
ALTER TABLE ads_account_health ENABLE ROW LEVEL SECURITY;
ALTER TABLE ads_ad_group_metrics ENABLE ROW LEVEL SECURITY;

-- Admin can see everything
CREATE POLICY "Admins can view all ads data" ON ads_campaign_metrics
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM users
...
CREATE POLICY "Clients can view own ads data" ON ads_campaign_metrics
  FOR SELECT USING (
    client_id IN (
...
CREATE POLICY "Admins can view all keyword data" ON ads_keyword_metrics
  FOR SELECT USING (EXISTS (SELECT 1 FROM users WHERE users.id = auth.uid() AND users.role = 'admin'));

CREATE POLICY "Clients can view own keyword data" ON ads_keyword_metrics
  FOR SELECT USING (client_id IN (SELECT id FROM clients WHERE id = (SELECT client_id FROM users WHERE id = auth.uid())));

CREATE POLICY "Admins can view all insights" ON ads_insights
  FOR SELECT USING (EXISTS (SELECT 1 FROM users WHERE users.id = auth.uid() AND users.role = 'admin'));

CREATE POLICY "Clients can view own insights" ON ads_insights
  FOR SELECT USING (client_id IN (SELECT id FROM clients WHERE id = (SELECT client_id FROM users WHERE id = auth.uid())));

CREATE POLICY "Admins can view all health scores" ON ads_account_health
  FOR SELECT USING (EXISTS (SELECT 1 FROM users WHERE users.id = auth.uid() AND users.role = 'admin'));

CREATE POLICY "Clients can view own health scores" ON ads_account_health
  FOR SELECT USING (client_id IN (SELECT id FROM clients WHERE id = (SELECT client_id FROM users WHERE id = auth.uid())));
...
-- Delete old keyword metrics after 1 year
-- Keep campaign metrics forever (they're small)
